...
function inferSystemName(vendor: string, resource: string): string {
  ...
  if (vendor === 'OpenAI' && lower.includes('gpt-4')) return 'Customer Support AI';
  if (vendor === 'Anthropic' && lower.includes('claude')) return 'Internal Copilot';
  if (vendor === 'Pinecone') return 'Vector DB – ' + resource;
  return resource || `${vendor} System`;
}

function inferSystemType(vendor: string, modelOrResource?: string): NormalizedAISystem['systemType'] {
  if (vendor === 'Pinecone') return 'Vector DB';
  const m = (modelOrResource ?? '').toLowerCase();
  if (m.includes('agent') || m.includes('copilot')) return 'Agent';
  if (m.includes('index') || m.includes('vector')) return 'Vector DB';
  return 'Model API';
}
